...
本项目的长期定位是：**本地优先、可插拔、可观测的 MCP RAG 框架**。核心职责是完成文档摄取、检索、重排、响应构建、评估与观测，并通过标准 MCP tools 将知识能力交给 MCP Host 或 Agent 使用。

明确的非目标：

- **不实现用户认证与授权**：不在 RAG 核心或 MCP Server 中增加登录、AuthN/AuthZ、RBAC、ACL 等能力。
- **不实现多租户与用户体系**：不维护用户、角色、组织、会话归属或租户级数据隔离。
- **不把项目演进为通用 Web 后端**：Stdio MCP 是主入口；可选 HTTP `KnowledgeService` 仅是可插拔的知识服务适配器，不改变项目定位。
- **安全边界外置**：本地访问由 MCP Host 与操作系统进程边界负责；如使用者自行对外部署，TLS、网络访问控制、认证与权限由反向代理、API Gateway 或外部业务系统负责，不进入本项目核心范围和排期。
- **Provider 凭据不属于用户认证**：LLM、Embedding 等 API Key 仅用于连接外部 Provider，仍需安全加载、避免写入日志和版本库。
...
- **范围约束**：本项目不实现 HTTP MCP 鉴权、多用户会话或多租户治理；这些能力属于 MCP Host 或外部部署层。
...
- **备选：FastAPI + 自定义协议层**
	- **场景**：仅在需要学习 MCP 协议底层细节时作为独立实验，不作为本项目主实现或排期目标。
...
> 2. **未来扩展性**：为更大知识库、更多文档和可插拔后端建立性能基线
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-07-28 | |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-07-28 | |
| A3 | 配置加载与校验（Settings） | [x] | 2026-07-28 | |

#### 阶段 B：Libs 可插拔层

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| B1 | LLM 抽象接口与工厂 | [x] | 2026-07-28 | |
| B2 | Embedding 抽象接口与工厂 | [x] | 2026-07-28 | |
...
| B7.4 | Ollama Embedding 实现 | [x] | 2026-08-01 | |
| B7.5 | Recursive Splitter 默认实现 | [x] | 2026-07-28 | |
| B7.6 | ChromaStore 默认实现 | [x] | 2026-07-29 | |
| B7.7 | LLM Reranker 实现 | [x] | 2026-08-01 | |
| B7.8 | Cross-Encoder Reranker 实现 | [x] | 2026-08-01 | |
| B8 | Vision LLM 抽象接口与工厂集成 | [x] | 2026-08-01 | |
| B9 | Azure Vision LLM 实现 | [x] | 2026-08-01 | |
...
| C4 | Splitter 集成（调用 Libs） | [x] | 2026-07-29 | |
| C5 | Transform 基类 + ChunkRefiner | [x] | 2026-07-29 | |
| C6 | MetadataEnricher | [x] | 2026-07-29 | |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| F1 | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2026-08-01 | |
...
| G5 | Ingestion 追踪页面 | [x] | 2026-08-02 | |
| G6 | Query 追踪页面 | [x] | 2026-08-02 | |

#### 阶段 H：评估体系

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| H1 | RagasEvaluator 实现 | [x] | 2026-08-02 | |
| H2 | CompositeEvaluator 实现 | [x] | 2026-08-02 | |
| H3 | EvalRunner + Golden Test Set | [x] | 2026-08-02 | |
| H4 | 评估面板页面 | [x] | 2026-08-02 | |
| H5 | Recall 回归测试（E2E） | [x] | 2026-08-02 | |

#### 阶段 I：端到端验收与文档收口

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| I1 | E2E：MCP Client 侧调用模拟 | [x] | 2026-08-02 | |
| I2 | E2E：Dashboard 冒烟测试 | [x] | 2026-08-02 | |
| I3 | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-08-02 | |
| I4 | 清理接口一致性（契约测试补齐） | [x] | 2026-08-02 | |
| I5 | 全链路 E2E 验收 | [x] | 2026-08-02 | |
...
### 7.1 部署边界与后端扩展
本项目保持本地优先的 Stdio MCP Server 定位。容器化可用于复现环境或封装依赖，但不会据此扩展用户体系、认证、权限或多租户能力。
- **Server 容器化**：可选提供 Dockerfile，用于环境隔离、依赖管理和可复现运行。
- **外部部署边界**：使用者若自行将服务暴露到网络，API Gateway、TLS、AuthN/AuthZ 和网络访问控制由外部部署层负责；本项目不实现这些模块。
- **后端扩展**：继续通过既有接口替换 Vector Store、Embedding、Reranker 和 `KnowledgeService`，不让部署方式侵入 MCP tools 与 RAG 核心。
...
- **业务逻辑驱动的生成控制**：
    ...
    - **挑战**：
        ...
        - **输出风格适配**：通过 Prompt 与响应格式配置控制答案的详略程度；不在 RAG 核心中识别用户身份或执行权限判断。
