...
export function IngestionPage() {
  ...
  const loadDocuments = useCallback(() => {
    api.getDocuments().then(setDocuments).catch(() => undefined)
  }, [])
  ...
  useEffect(() => {
    Promise.all([api.getIngestionOptions(), api.getActiveJob(), api.getDocuments()]).then(([nextOptions, activeJob, docs]) => {
      ...
      setJob(activeJob)
      setDocuments(docs)
       if (nextOptions.collections.length) setCollection((current) => nextOptions.collections.includes(current) ? current : nextOptions.collections[0])
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      api.getIngestionJob(job.job_id).then((nextJob) => {
        setJob(nextJob)
