...
export function AppShell({ path, onNavigate, children }: { path: string; onNavigate: (path: string) => void; children: ReactNode }) {
  ...
  return <div className={`app-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
    <aside className={`sidebar ${mobileOpen ? 'sidebar-mobile-open' : ''}`}>
      ...
      <nav className="main-nav">{navigation.map(({ to, label, icon: Icon }) => <a key={to} href={to} onClick={(event) => navigate(event, to)} className={`nav-item ${path === to ? 'nav-item-active' : ''}`} aria-current={path === to ? 'page' : undefined} title={collapsed ? label : undefined}><Icon size={18} /><span>{label}</span></a>)}</nav>
      <div className="sidebar-bottom"><div className={`sidebar-status sidebar-status-${health}`}><div className="sidebar-status-head"><span><i className={`health-dot health-${health}`} />API 服务</span><small>LIVE</small></div><strong>{liveStatusLabels[health]}</strong><span>每 5 秒自动检查连接</span></div><button className="collapse-button" onClick={() => setCollapsed(!collapsed)}>{collapsed ? <PanelLeftOpen size={17} /> : <PanelLeftClose size={17} />}<span>{collapsed ? '展开' : '折叠'}侧边栏</span></button></div>
    </aside>
