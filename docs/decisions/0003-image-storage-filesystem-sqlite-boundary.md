...
└── 生成 ImageRef
        │
        ▼
ImageStorage.save_refs(ImageRef[])
├── 校验图片已经存在且路径受管
...
SQLite 的 `image_index` 表保存：

| 字段 | 含义 |
|------|------|
| `image_id` | 图片唯一身份和 upsert 主键 |
...
整批通过后开启 SQLite 短事务
    │
    ▼
INSERT ... ON CONFLICT(image_id) DO UPDATE
```

先校验完整批次，再开始数据库写入，保证某一张图片不合法时不会把同批次前面的合法
映射提前写入。相同 `image_id` 再次保存会更新原行，不会增加重复映射。
...
文档级删除使用 `source_path + collection` 作为范围：
...
当前选择“先提交索引删除，再清理文件”。原因是文件清理失败时，留下不再被系统引用
...
6. C14 装配时必须让 `PdfLoader.image_root` 与 `ImageStorage.image_root` 来自同一配置。

## 已知顾虑与残余风险

| 顾虑 | 可能后果 | 当前控制 | 后续处理 |
|------|----------|----------|----------|
| Loader 写出 PNG 后、SQLite 入库前进程崩溃 | 产生没有索引行的孤立图片 | 图片 ID 和路径确定，可通过重新摄取覆盖同一文件 | C14 定义失败补偿；有实际积累后增加“文件对 SQLite”清理工具 |
| SQLite 删除成功后 PNG 删除失败 | 产生不再被引用的孤立图片 | 删除顺序不会留下有效索引指向缺失文件 | 向调用方传播文件错误；需要时增加孤立文件巡检和重试 |
| PNG 被人工删除但索引行仍存在 | `get()` 仍返回 ImageRef，真正读取图片时失败 | 保存阶段检查文件存在；文档删除和重新摄取可恢复 | Dashboard 或 C14 增加索引健康检查前，不声称读取时自动修复 |
| 文件系统与 SQLite 没有联合事务 | 断电或异常可能留下两边不一致 | SQLite 自身事务完整；失败结果偏向可重新摄取或清理的状态 | 不把 WAL 描述为跨存储事务；更强一致性需求出现时重新设计 |
| 同一 image ID 被更新到新路径 | SQLite 只保留新路径，旧 PNG 可能成为孤立文件 | 正常 Loader 使用确定性 ID 和路径，不会主动移动 | 允许图片迁移时增加旧路径清理或禁止 ID 改绑 |
| 数据库保存绝对路径 | 项目目录整体移动后，已有映射可能失效 | 当前定位是单机固定工作区 | 需要可搬迁索引时改存相对 `image_root` 的路径并提供迁移 |
| `source_path` 使用调用方传入字符串 | 同一 PDF 的相对、绝对或符号链接路径可能被视为不同文档 | 当前 Pipeline 在同一轮中沿用 Loader 的路径字符串 | C14 统一路径规范化规则，并以 file hash 辅助文档身份 |
| 多线程 save/delete 同一文档 | WAL 保证数据库不写坏，但业务结果取决于操作顺序 | 单个 SQL 事务原子，路径删除有安全检查 | C14 的文档领取机制覆盖摄取；删除和摄取仍需文档级协调 |
| SQLite 表缺少显式 schema version | 将来字段变化时难以判断是否需要迁移 | C13 是首个版本，当前不存在历史 schema | 首次结构升级时增加 `schema_version` 和迁移测试 |
| `list_by_collection()` 一次返回全部记录 | 大 collection 可能增加内存和 UI 延迟 | 当前本地小规模语料 | 实测出现压力后再增加分页，不提前扩展接口 |
| 磁盘没有配额和自动垃圾回收 | 大量图片或孤立文件可能耗尽空间 | 图片按 collection 集中存放，支持文档级删除 | Dashboard 统计或运维需求出现时增加容量监测与清理策略 |
...
- 测试覆盖幂等 upsert、按文档/collection 查询和跨 collection 删除隔离。
