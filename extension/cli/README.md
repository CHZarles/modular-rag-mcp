...
`upload` 只接受最大 50 MiB 的 PDF，并将文件编码为 Base64 后调用 MCP `upload_document`。命令返回 `job_id`，Agent 使用 `job` 轮询 `queued`、`running`、`success`、`skipped` 或 `failed`；CLI 不直接调用摄取 Pipeline。
